...
public class UITexture : UIWidget
{
    ...
    [ContextMenu("Test ETC Texture")]
    public void SetETCMat(string kTexName = "", string kShaderName = "")
    {
        if (mTexture == null)
            return;
        if (string.IsNullOrEmpty(kTexName))
            kTexName = mTexture.name;
        if (string.IsNullOrEmpty(kShaderName))
            kShaderName = shader.name;

        Material _mat = new Material(Shader.Find(kShaderName));

        _mat.SetTexture("_MainTex_Alpha", GameCommon.GetTexutreByName(kTexName + "_Alpha"));
        mMat = _mat;
