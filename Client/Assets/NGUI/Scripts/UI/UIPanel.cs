...
public class UIPanel : UIRect
{
	...
	protected override void OnInit ()
	{
        ...
        if (forceUpdateOnStart && mUpdateFrame == Time.frameCount && mUpdateTime < RealTime.time - 0.0001f)
        {
            ForceUpdateOnStart();
        }
	}
	...
	void LateUpdate ()
	{
		if (mUpdateFrame != Time.frameCount)
		{
            ...
            //!!! Modify by Yang Wenge, for render effect between UI window
			int rq = 2900;
    ...
    /// <summary>
    /// add by ZhuKai
    /// 如果此Panel的Start已经错过当前帧的刷新时序，则强制其在Start时立刻调用刷新的方法，以避免刷新延迟到下一帧的问题
    /// </summary>
    public bool forceUpdateOnStart { get; set; }

    private void ForceUpdateOnStart()
    {
        UpdateSelf();
