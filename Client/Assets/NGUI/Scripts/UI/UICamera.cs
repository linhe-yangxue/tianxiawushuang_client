...
public class UICamera : MonoBehaviour
{
    ...
    private static int mCurFrameCount = 0;  //> 当前帧数计数器
    public static bool mCanClick = true;    //> 当前是否可以点击
    public const int FRAME_STEP = 3;       //> 10帧之内只能接受一次点击事件[原来是10帧]
    private static bool mNeedCheckClick = true; //> 是否需要检测点击频率
    public static bool NeedCheckClick
    {
        set { mNeedCheckClick = value; }
        get { return mNeedCheckClick; }
    }
	void Update ()
	{
			...
			if (currentTouch.pressed != null)
			{
					...
					currentTouch.totalDelta.magnitude < drag))
				{
					...
					if (currentTouch.clickNotification != ClickNotification.None)
					{
                        ...
                        //ScreenLogger.Log("Click " + currentTouch.pressed.name);
