Shader "UI/Lit/Refraction Detail (Pro Only)"
{
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			{
				fixed4 col = tex2D_ETC1(_MainTex,_MainTex_Alpha, IN.texcoord1.xy);
				fixed4 detail = tex2D_ETC1(_DetailTex,_MainTex_Alpha, IN.texcoord2.xy);

				half3 normal = UnpackNormal(tex2D(_MainBump, IN.texcoord1.zw)) +
							   UnpackNormal(tex2D(_DetailBump, IN.texcoord2.zw));
				
				half3 mask = tex2D_ETC1(_Mask,_MainTex_Alpha, IN.texcoord1.xy) *
							 tex2D_ETC1(_DetailMask,_MainTex_Alpha, IN.texcoord3);
